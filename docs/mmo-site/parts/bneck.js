/* ═══════════ 병목 탭 — 실험 카드 열여섯 장 + 계기판 ═══════════
   한 화면에서 끝난다: 지도(왼쪽) · 카드 열여섯 장과 계기판(오른쪽).
   지도는 위 코드가 이미 갖고 있다. 여기서 더하는 것은 둘뿐이다 —
     ① 카드 열여섯 장: 번호 · 판정 · 이름 · 동접 고리 (노션 실측 보고서와 1:1)
...
/* 배열 순서가 곧 판의 자리다 — 세 칸씩 두 줄(bneck.css 의 .bx-dash).
   ...
   (⑪ 10,890,000 건/s × 92.4B = 1,006MB/s · ⑮ 17,620,000 × 75.2B = 1,325MB/s.
    값이 있는 카드는 전부 이 식이 맞는다 — ⑫만 그 페이지가 트래픽을 안 적어 셋이 비어 있다).
...
/* 카드에 서는 것은 노션 실측 보고서가 있는 열여섯뿐이다(scene.js 의 NOTION_EXP 가 no·nt 를 붙인다).
...
function fmtV(k,v,d,big){
  ...
  /* 소켓 호출은 2.4만~13.6만 사이에 다 들어간다. pps 처럼 1e6 에서 단위를 갈면 열여섯이 전부 k 인데
     ...
     '몇 만 번대인가' 뿐이라 한 자리면 충분하고, 그래야 열여섯 장을 세로로 훑을 때 자가 안 바뀐다. */
...
/* ═══ 막대의 눈금 = 지금 ═══
   ...
   이제 눈금은 하나다: **현황판(지금 참인 값)이 100%**. 열여섯 실험이 같은 자 위에 서므로
   막대 길이가 곧 '이 실험이 어느 부하 지점이었나' 가 된다 — ① 은 거의 비어 있고 뒤로 갈수록 찬다.
   그 자체가 이 탭의 이야기다(동접 200 에서 7,200 까지 부하를 올려온 순서).
   ...
   ② 눈금을 넘는 값은 100% 에서 자르되 칸에 over 를 붙여 끝을 막는다. 안 그러면 '딱 맞게 찼다' 와
      '넘쳐서 잘렸다' 가 같은 그림이 된다.
   값이 있는데 눈금 대비 1% 도 안 되는 칸(① 송신량 = 1.62/969)은 최소 폭을 남긴다. 0 으로 두면
   값이 없는 칸(⑤ 소켓 호출)과 그림이 같아진다.

   ※ 눈금이 '지금' 이라 **옛 카드가 넘치는 것은 정상이다.** 엔트리 9B 로 바이트를 깎은 것이
     지금이므로, 14B 시절 카드(⑪ 송신량 1,006 · ⑬⑭ 1,240~1,249)는 눈금 969 를 넘어 over 가 붙는다.
     '옛날이 더 컸다' 가 사실이고 그림도 그렇게 읽혀야 한다. */
...
var SCALE={
  ...
  /* 이 둘도 현황판(NOWE)에서 온다 — 여섯 칸의 100% 는 한 지점이어야 한다.
     결론 카드(SUME)로 물러서는 뒷길을 남긴 것은 현황판 부하 행이 없던 시절의 자리다.  */
  pkt : NOWE&&NOWE.ld ? NOWE.ld.pkt  : (SUME&&SUME.ld ? SUME.ld.pkt  : null),
  sock: NOWE&&NOWE.ld ? NOWE.ld.sock : (SUME&&SUME.ld ? SUME.ld.sock : null),
...
function paintDash(it){
  tiles.forEach(function(T){
    ...
    T.fr.textContent=(!num ? ""            /* 범위 값 — 왼쪽에 또 같은 문자열을 적지 않는다 */
      ...
      /* 값은 서 있는데 **이 실험이 잰 것이 아닌** 칸 — '기록 없음'(값 자체가 없다)과 다르다.
         ③④ 평균 패킷이 그렇다 : avg_pkt_bytes 지표가 2026-06-22 에 생겼고 두 런은 그 전이라,
         같은 프로토콜인 ①② 의 실측을 빌려 세웠다(이 판의 유일한 가정). 빌린 값을 '이 실험의 부하'
         라고 적으면 이 실험에서 잰 것으로 읽히므로 그 자리만 말을 바꾼다.
         ※ 반드시 아래 M.ld 분기 **앞**에 온다 — 뒤에 두면 '이 실험의 부하' 가 먼저 걸려 무효가 된다. */
      : (M.ld && it.ldnote && it.ldnote[M.k]) ? it.ldnote[M.k]
      : it.st==="dg" ? "이 부하에서 관측"
      /* 값이 아예 없으면 왼쪽도 비운다 — '지금 값' 이라고 적어 놓고 오른쪽이 '—' 면
         '지금 값이 없다' 로 읽힌다. ⑯ 결론 카드의 틱이 그렇다(버킷 보간이라 인용 금지라
         비워 둔 자리다). 값이 있는 sum 칸은 그대로 '지금 값' 이다. */
      : it.st==="sum" ? (av==null ? "" : "지금 값")
      : M.ld ? "이 실험의 부하"          /* 켜고가 없는 부하 값 — 견줄 짝이 없다 */
      /* 여기까지 안 걸렸는데 값은 있다 = A/B 짝이 없는 단일 관측이다. ⑫ 가 그렇다 —
         판정은 기각인데 그 페이지가 A/B 를 트래픽으로 적지 않아 끄고가 통째로 없다.
         이 줄이 없으면 동접 4,800 이 뻔히 있는데도 아래 '이 시기 미수집' 으로 떨어진다
         (2026-08-18 화면에서 확인). ※ 이 줄은 반드시 dg·sum·ld **뒤**에 와야 한다 —
         앞에 두면 ② 평균 패킷('이 실험의 부하')처럼 이미 제 이름을 가진 칸을 덮는다. */
      : av!=null ? "이 부하에서 관측"
      : "이 시기 미수집");
...
/* ═══ 카드 열여섯 장 ═══
   ...
   결과 수치는 카드에 없다 — 한때 '송신 호출 −94%' 같은 성과 한 줄을 넣었는데, 열여섯 장이
   저마다 다른 지표를 외치면 훑을 때 이름이 뒤로 밀리고 서로 견줄 수도 없었다(실험마다 잰 지표가 다르다).
   무엇이 얼마나 바뀌었나는 아래 계기판이 고른 한 장에 대해서만 말하고, 나머지는 노션 원문이 맡는다.
   색도 고른 카드에만 켠다 — 열여섯이 저마다 판정색을 띠면 어느 것이 열려 있는지가 색으로 안 짚인다.

   ↗ 도 카드에서 걷어냈다 — 열여섯 장이 같은 기호를 하나씩 달고 있었는데 열리는 것은 고른 하나뿐이라,
   판 아래 한 짝(#bn-more)으로 모으고 주소만 갈아 끼운다(아래 show).
   고리의 눈금은 계기판 막대와 같은 SCALE.ccu(현황판) 다 — 같은 지표에 자를 두 벌 두지 않는다.
   숫자를 지운 자리라 값은 aria-label 로 남긴다(화면에는 라벨 줄의 '200 → 7,200' 이 눈금을 말한다).
   ※ 고리가 꽉 차는 카드는 없다 — 눈금 7,200 은 천장이고, 카드에 찍히는 것은 그 실험의 A/B 부하점이라
     가장 높은 ⑮⑯ 도 6,600(92%)이다. 100% 는 '거기까지 재 봤다' 가 아니라 '지금 버티는 선' 이다.
     ⑮ 가 5,600 이던 시절에는 그 최댓값이 78% 였다 — 2026-08-20 재측정으로 6,600 이 되며 올라왔다. */
function ringPct(ccu){
  var top = SCALE.ccu || 7200;
...
/* 팬아웃 = 한 번 보낼 때 대상이 몇 명인가. 열여섯 실험이 전부 OFF/ON 을 같게 맞추고 재는 통제
   지표라 '끄고 → 켜고' 가 없다 — 계기판에 칸으로 세우면 홀로 화살표가 없어 빈 칸으로 보인다.
   그래서 라벨 줄 오른쪽에 조건으로 적는다(동접이 그렇듯 이것도 '어떤 부하였나' 에 속한다). */
/* hint = 이 카드의 여섯 칸을 곧이곧대로 읽으면 안 되는 사정. 여섯뿐이라 상수로 두지 않고
   ...
   둘 다 '이 값들이 어떤 조건에서 나온 것인가' 라 한 줄에 같이 서는 것이 맞다.
     ...
     ⑭ 안정 구간 대조라 경계에서의 결과는 이 카드가 말하지 않는다는 것 */
