...
/* ═══ 규격 세 값 ═══
   ...
   값은 규격이라 각주 줄(.f)이 필요 없다. 숫자 없는 칸(.t 가지)은 '더미 부하' 가 꼬리줄로 내려가며
   쓸 곳이 없어졌지만 가지는 남겨 둔다 — 값 하나 바꾸려고 틀을 다시 짜지 않게.
   판이 놓이는 자리는 panel-client.html 이 들고 있다 — 여기는 값만 채운다.
   ※ 판 머리줄('규격 · 세 화면 공통')은 걷어냈다. 액자 머리줄이 같은 말을 이미 한다.

   ── 여섯을 셋으로 줄였다 ──
   남기는 기준은 하나다: 화면을 봐도 알 수 없는 값만 남긴다.
     · 시야 3×3     → 캡션 첫 문장이 '내 섹터와 이웃 여덟' 이라고 더 크게 말한다(중복)
     · 콘솔 화면    → 이 컷 하나의 규격이라 ② 의 이름줄 옆으로 내려보냈다
     · 더미 부하    → 숫자가 아니라 계열이 깨지던 칸. 꼬리줄의 '부하 더미를 붙이고' 뒤가 제 자리다
   셋이 빠지면서 판이 두 줄에서 한 줄로 줄고, 그 높이는 통째로 ③(덮임컷)이 가져간다 —
   이 탭에서 유일하게 결론을 말하는 컷('서버는 버텼고 화면이 덮였다')이 가장 작던 것을 되돌린다.

   ── 단위를 계통으로 맞췄다 ──
   맵만 단위가 없었다(120×120 / 20 칸 / 3×3 섹터). 셋 다 '칸' 으로 세는 값이므로 맵에 칸을 붙이고,
   섹터도 20 이 아니라 20×20 으로 적는다 — 맵 120×120 과 같은 문법이라 120÷20=6, 6×6=36 이
   눈으로 이어진다(값의 근거는 Run/bin/IOCP_ServerConfig.ini 의 Width/Height/SectorSize).
   '36개' 는 단위가 아니라 라벨로 붙인다 — 단위 자리에 '칸 · 36개' 로 두면 값이 20×20 으로
   길어진 만큼 칸 폭(약 109px)을 넘겨 '36개' 만 아랫줄로 접힌다(1440px 창에서 확인). */
(function(){
  var rows=[["맵","120×120","칸"],
            ["섹터 36개","20×20","칸"],
            ["서버 틱","40","ms"]];
