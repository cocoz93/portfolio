...
/* ── 글자 배율 ──
   ...
   1.36 을 곱하면 8.5→17.2 · 10→20.3 · 11.5→23.3 이다. 1-1 이 최소 18 · 본문 19~25 이므로
   이 대역이 곧 그쪽과 같은 자다(1.25 로는 본문이 18.6 이라 아직 한 뼘 작았다).
   ※ 좌표는 이 배율에 맞춰 다시 잡았다 — 글자만 키우면 줄끼리 붙는다. */
var LOAD_FS=1.36;
...
function loadEnv(g){
  ...
  var bw=54, st=66, xs=24, xc=604;                 /* 서버 24..408 · 틈 196 · 클라 604..856 */
  for(var i=0;i<10;i++){
  ...
  g.appendChild(el("path",{d:"M 435 81 L 577 81",stroke:"#6cc7ff","stroke-width":"1.6",
    fill:"none","stroke-dasharray":"5 4"}));
  g.appendChild(el("polygon",{points:"592,81 577,74 577,88",fill:"#c98a34"}));
  g.appendChild(el("polygon",{points:"420,81 435,74 435,88",fill:"#6cc7ff"}));
  /* 코어 아래 라벨 셋(ServerCores · loopback · ClientCores)은 같은 y 에 선다 — 높이가
     어긋나면 가운데 것만 계단처럼 내려앉아 딴 층의 글로 읽힌다 */
  tx(g,506,126,"loopback 127.0.0.1",8.5,"#8496b3","700","middle","var(--mono)");
  tx(g,506,148,"랜선도 공유기도 안 탄다",8.5,"#5d6c85","600","middle");
...
function loadMap(g){
  ...
  [[310,"시야가 맵의","25%","#6cc7ff","36칸 중 9칸이 내 시야"],
   [592,"한 사람이 받는 팬아웃","922","#ffb648","동접 5,000 기준 실측"]].forEach(function(t){
    bx(g,t[0],54,264,146,"#0f1522","#1e2739",11);
    tx(g,t[0]+22,90,t[1],10,"#8496b3","700");
    tx(g,t[0]+22,142,t[2],30,t[3],"800",null,"var(--mono)");
    tx(g,t[0]+22,178,t[4],8.5,"#5d6c85","600");
  });
  /* 계기 칸 바로 아래에 붙인다. 한때 왼쪽 격자 캡션과 같은 y(300)에 뒀는데, 글자가 커지자
     둘이 한 줄에 나란히 서서 '…받지도 않는다 높아야 적은 동접으로…' 로 이어 읽혔다. */
  tx(g,310,236,"시야가 이렇게 넓은 것은 의도다 — 실제 게임보다 밀도가",10,"#8496b3","700");
  tx(g,310,262,"높아야 적은 동접으로 서버 병목이 빨리 드러난다.",10,"#5d6c85","600");
}
...
function loadBot(g){
  ...
  for(var t=0;t<5;t++){
    var ty=54+t*34;
    bx(g,24,ty,32,24,"#3a2c14","rgba(255,182,72,.45)",5);
    tx(g,40,ty+17,"#"+t,10,"#ffcf8a","800","middle","var(--mono)");
    for(var w=0;w<10;w++) bx(g,70+w*17,ty+7,13,10,"#1f4f74",null,3);
    tx(g,258,ty+17,"1,000명",10,"#8496b3","700",null,"var(--mono)");
    tx(g,410,ty+17,"소켓 1,000",9,"#5d6c85","600","end","var(--mono)");
  }
  /* 한 줄로 붙인다 — 오른쪽 꼬리줄(하트비트)과 같은 높이에서 끝나야 판 아래가 고르다 */
  tx(g,24,264,"스레드를 늘리면 서버 코어를 뺏는다 — 담당 인원을 늘리는 쪽을 택했다.",10,"#5d6c85","600");
  /* 한 판 안에 이야기가 둘이라 세로선으로 끊는다 */
  g.appendChild(el("line",{x1:460,y1:40,x2:460,y2:286,stroke:"#26324a","stroke-width":"1"}));
  tx(g,490,28,"매 틱 주사위 하나 — 40ms 마다",11.5,"#c3cfe0","800");
  tx(g,856,28,"끝 5% = 존 이동",8.5,"#6f88ad","700","end");
  function bar(y,label,segs){
    tx(g,490,y-12,label,10,"#8496b3","700");
    var x=490, W=366;
    segs.forEach(function(sg){
      var w=W*sg[0]/100;
      g.appendChild(el("rect",{x:x,y:y,width:w,height:34,fill:sg[1],
        rx:(x===490||x+w>=490+W-1)?4:0}));
      if(w>44) tx(g,x+w/2,y+23,sg[3],10,sg[2]||"#0d1220","800","middle","var(--mono)");
      x+=w;
    });
  }
  /* 초록은 여기서 뺐다 — 초록은 4 번 판의 '합격' 색이다. 같은 이유로 '정지' 의 빨강도 걷었다 */
  bar(80,"서 있을 때",[[75,"#6cc7ff",null,"이동 75"],[20,"#ffb648",null,"채팅 20"],[5,"#4e6f9c",null,"5"]]);
  bar(176,"걷고 있을 때",[[20,"#5b7194",null,"정지 20"],[80,"#2b3a52","#c3cfe0","계속 이동 80"]]);
  /* '확률이 따로 도는 게 아니라…' 는 여기 있었는데 판 설명과 글자까지 같아 걷어냈다 */
  tx(g,490,264,"하트비트는 20초 고정이고 서버 타임아웃은 60초다.",10,"#5d6c85","600");
}
