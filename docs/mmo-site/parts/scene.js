...
(function(){
  ...
  const EXPS=[
    ...
    {s:0.3, loc:"0", n:"정정 이력", st:"fix",
     r:"뒤집힌 결론도 지우지 않는다",
     cond:"착시(당시 측정이 틀림) 1건 · 뒤집힘(당시엔 맞았고 이후 최적화가 무효화) 3건",
    ...
    {s:5, loc:"1", n:"1,500 동접 붕괴 — 회선 상한", st:"dg",
     ...
     bm:null, am:{ccu:1500,pps:2100000,send:41,tick:19},
     abh:"동접 1,000 → 1,500",
     ab:[["송신 경합","0.9","5,533 /s","폭증",-1],
         ["송신 큐 넘침","0","26 /s","발생",-1],
    ...
    {s:7, loc:"1", n:"IOCP 워커 12 → 4", st:"ok",
     ...
     bm:{ccu:4000,pps:8700000,send:615,tick:66.4}, am:{ccu:4000,pps:8700000,send:615,tick:54.5},
     abh:"워커12·송신2 → 워커4·송신2",
     ab:[["왕복 p99","143","102 ms","−28%",1],
         ["주기 평균","40.3","35.1 ms","−13%",1],
         ["3회 편차","48~185","0.2 ms","재현성",1],
         ["시스템 CPU","2.98","2.88 코어","−3%",1]]},
    ...
    {s:8, loc:"2", n:"멤버십 팬아웃 묶음", st:"ok",
     ...
     bm:{ccu:4000,pps:8710000,send:614,tick:59.7}, am:{ccu:4000,pps:6540000,send:606,tick:39.6},
     ab:[["JOIN/LEAVE / 틱","19.5","6.2 ms","−68%",1],
         ["틱 평균","36.2","20.4 ms","−44%",1],
         ["게임루프 CPU","0.86","0.49 코어","−43%",1],
         ["평균 패킷 크기","71","93 B","+30%",1]]},

    {s:9, loc:"3", n:"브로드캐스트 수신섹터 묶음", st:"ok",
     r:"복사 −88% · 게임루프 0.57→0.21 코어",
     cond:"동접 4,000 · 팬아웃 738 tgt/call · 대상 27만 회/틱 · 3rep 중앙값",
     m:"대상마다 세션을 붙잡고 검사하던 고정비를, 받는 섹터별로 이어 붙여 한 번에 접었다",
     ex:"세션당 넣기가 27만 → 5천으로 줄었다. 틱 p99 는 −52% 인데 송신량은 −2.6% — 접은 것은 CPU 고정비이지 나가는 바이트가 아니다",
    ...
    {s:10, loc:"1", n:"송신 워커 K2 → K3", st:"ok",
     ...
     cond:"동접 5,000 · 워커4 고정 · 3rep 중앙값 · 단일 PC 루프백",
  ...
  /* ═══ 측정 부하 — 노션 각 페이지 콜아웃의 '측정부하' 토글에서 옮겨온 값 ═══
     ...
     null 은 '그 페이지가 그 값을 안 적었다' 이지 0 이 아니다. 셋뿐이고 이유가 다 다르다:
       ⑤ 1,500 붕괴 — 이 런만 원본 지표가 안 남아 부하가 실측이 아니라 환산이다(소켓 호출은 아예 없다)
       ⑦ IOCP 워커  — 부하 줄이 트래픽 대신 스레드별 CPU 로 적혀 있다(송신 1.00 포화가 그 페이지의 부하점)
       나머지 빈칸  — '변경 후' 를 안 적은 것, 곧 그 실험이 그 값을 안 건드렸다는 뜻 */
  const LOAD=[
    [1,     37, 19.7, null,  82600, null],
    [2,     37, 19.7, null,  82100, 4900],
    [3,    184, 19.7, null,  24400, null],
    [4,    184, 19.7, null,  24400, null],
    [5,    223, 19.7, null,   null, null],
    [6,    370, 19.6, 67.1,  51400, 48800],
    /* ⑦ 평균 패킷은 노션 부하 줄에 없다(그 페이지는 트래픽 대신 스레드별 CPU 를 적었다).
       비워 두다가 실측 CSV 에서 되찾았다 — Monitoring/metrics_out/window_metrics.csv 의
       C4000_WT12_K2 3회 중앙값 70.62 · C4000_WT4_K2 3회 중앙값 70.94(session_count 4,000).
       라벨이 노션의 '워커{12,4} × 송신{2,4} × 각 3회' 와 그대로 맞고, 같은 동접인 ⑧ 의 71 B 와도
       어긋나지 않는다. 워커 수를 바꿔도 +0.4% 라 이 칸은 통제 지표였다는 것이 값으로 드러난다. */
    [7,    463, 70.6, 70.9,  55000, null],
    [8,    738,   71,   93,  98300, 98000],
    [9,    738, 92.2, 94.0,  98800, 97500],
    /* ⑩ 만 소켓 호출이 범위다 — K2·K3·K4 를 다 잰 실험이라 노션도 범위로 적었다(99k~122k/s).
       가운데 하나를 골라 숫자로 만들면 원문에 없는 값이 된다. 문자열이면 그대로 찍힌다.
       단위는 노션 원문대로 k 다 — 다른 열한 칸도 화면에서는 k 로 찍히므로 같은 자를 쓴다. */
    [10,   923, 93.4, null, "99k~122k", null],
    [11,   922, 93.5, null, 123000, null],
