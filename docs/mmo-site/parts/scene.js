...
(function(){
  ...
  /* ═══ 실험 데이터 — 노션 [병목지점 개선] 이 정본이다 ═══
     ...
     실험마다 잰 동접이 다르다(200 → 7200). 그래서 실험끼리 값을 이어 붙이면 안 되고,
     계기판도 '그 실험 한 건의 끄고 → 켜고' 만 보여준다.
       st  ok 채택 · rj 기각 · dg 진단(판정 없음) · na 병목 아님(전체 계측에서 안 잡혀 전용 A/B 를 안 돌린 자리)
  ...
  const EXPS=[
    /* ── 자리 0 = 현재. 실험이 아니라 '지금 무엇이 참인가' 를 모아 두는 칸이다.
       각 실험 카드가 보여 주는 것은 그 실험을 돌린 시점의 값이라, 마지막 카드의 7,200 을
    ...
    {s:0, loc:"0", n:"현황판", st:"now",
     ...
     /* 여섯 칸이 전부 **동접 7,200 한 지점**이다(2026-08-19 통일).
        한때 ccu·tick 만 7,200 이고 트래픽은 5,600 값이라 기준이 섞여 있었다 — 노션 결론 페이지가
        트래픽을 5,600 으로만 적었기 때문인데, 실측 CSV 에 7,200 런이 남아 있었다.
        출처 : `RMDU_IOCP_D1_CC7200_r1` + `RMDU3_IOCP_D1_CC7200_r1` 2rep 평균.
        ※ 3rep 중 `RMDU_r2` 는 뺐다 — 부하클라가 불건강해서다(loop_p99 45.74). 노션 ⑮ 4장이
          "3rep 중 1rep 는 클라 불건강(loop_p99 45.7)으로 판정 제외" 라고 적은 그 rep 이고,
          남은 둘의 tick_avg 평균이 22.11 로 노션의 22.1 과 맞아 라벨이 확인된다. */
     bm:null, am:{ccu:7200,pps:20116000,send:1499,tick:39.7},
     src:{bm:null, am:"RMDU_IOCP_D1_CC7200_r1+RMDU3_IOCP_D1_CC7200_r1"},
    ...
    {s:0.1, loc:"0", n:"동접별 트래픽", st:"now",
     ...
     /* 옛 사다리(6,000 −13% · 6,500 −24%)를 통째로 걷어냈다. 그 회차들은 측정 환경이 같은 일에
        CPU 를 19~21% 더 쓰고 있던 오염 구간이라 절대값을 인용할 수 없다(노션 정정 이력 6).
        대신 깨끗한 환경의 천장 사다리를 싣는다 — 같은 판정 기준으로 14B 와 9B 를 나란히 둔 표라,
        '어디서 갈렸나' 가 한 줄씩 읽힌다. −24% 는 정정 이력 카드가 −20% 로 따로 보관한다. */
     /* 눈금과 같은 7,200 실측을 쓴다(현황판과 같은 런 — RMDU 유효 2rep). 아래 사다리가
        "7,200 까지 갔다" 를 말하는 카드라, 배지 값도 그 끝점이어야 앞뒤가 맞는다. */
     bm:null, am:{ccu:7200,pps:20116000,send:1499,tick:39.7},
     src:{bm:null, am:"RMDU_IOCP_D1_CC7200_r1+RMDU3_IOCP_D1_CC7200_r1"},
    ...
    {s:0.2, loc:"0", n:"결론 — 세트 복제로", st:"sum",
     /* '근본 벽' 은 노션 결론 페이지의 🎯 콜아웃 문구를 그대로 쓴다 — 2026-08-18 개편에서
        '팬아웃' → '브로드캐스트' 로 바뀌었다(같은 것을 가리키는 다른 이름이지만, 정본이
        고른 말을 따른다). 자리 2 핀 이름도 이 축과 같은 것을 말한다. */
     r:"세트 2.4코어로 5,000명 · 근본 벽은 브로드캐스트",
     ...
     /* A/B 가 아니라 종합이라 끄고(bm)가 없다 — 계기판은 이 null 을 보고 '지금 값' 으로 적는다.
        켜고 자리의 넷은 ⑮ 9B 페이지 3절 실측이고, 그 페이지가 2026-08-20 에 부하점을 5,600 →
        **6,600** 으로 다시 재면서 이 카드도 같이 옮겼다. 결론이 ⑮ 와 같은 런을 말하게 두는 쪽이
        중요해서다 — 한 화면에 두 카드가 나란히 서는데 서로 다른 부하점을 '지금 값' 으로 부르면
        어느 쪽이 현재인지 읽을 수가 없다.
        ※ 그 전에는 5,600 · 8/16 값(1,299만 · 970 · 74.6 B)이었고, 더 전에는 결론 페이지의
          5,599 · 8/15 값(1,298만 · 969 · 74.7 B)이었다. 셋 다 같은 9B 를 잰 것이고, 갈리는 것은
          부하점뿐이다. 결론 페이지 쪽 값은 지금도 노션에 살아 있다(이력으로 남는다).
        ※ 위 현황판(s:0)과는 여전히 부하점이 다르다 — 그쪽은 눈금이라 용량(7,200) 기준이고
          여기는 A/B 실측이 있는 6,600 기준이다. 현황판은 카드로 서지 않으므로 겹치지 않는다. */
     /* 틱은 한때 비어 있었다 — 노션 결론 페이지가 "히스토그램 버킷 보간이라 정밀 인용 금지" 라고
        못 박아 p99 를 안 실었기 때문이다. 6,600 재측정에서는 ⑮ 3절 표가 p99 를 직접 싣는다
        (38.5 → 29.9). 그 경고가 걸린 것은 결론 페이지의 CC5,500 틱 평균 쪽이고, 그 값(10.4ms)은
        인용해도 되는 값이라 아래 ab 에 그대로 둔다. */
     bm:null, am:{ccu:6600,pps:17620000,send:1326,tick:29.9},
     src:{bm:null, am:"notion:3bb16a0b9f598049a873fad65fc1aeff#3절·9B"},
     abh:"항목 → 값 (동접 6,600 · 8/20 재측정)",
     /* 옛 '맵당 3.4코어 · 코어당 ~1,470명' 은 5,000 시절 값이다. 지금 노션은 확장 단위를
        '세트(IOCP 4 + 게임 1 + 송신 3 + DB 1) = CC5,000 실측 2.4코어' 로 적고, 그래서
        코어당 수용도 5,000 ÷ 2.4 = 약 2,080명이 된다. 현황판 카드도 같은 값을 쓴다. */
     /* 이 다섯 줄은 계기판과 달리 **노션 결론 페이지의 값**이고, 부하점이 줄마다 다르다
        (세트 5,000 · 틱 평균 5,500 · 회선 환산 5,600). 그래서 부하점이 결론과 다른 줄에는
        그 값을 적어 둔다 — 계기판이 6,600 이라고 이 줄들까지 6,600 으로 고쳐 쓰면 노션에
        없는 수를 지어내게 된다. 회선 환산은 6,600 에서 112% 이고, 그 값은 ⑮ 카드에 있다. */
    ...
    {s:0.3, loc:"0", n:"정정 이력", st:"fix",
     ...
     src:{bm:null, am:null},
     abh:"실제",
     /* 노션 하위 페이지 「정정 이력 — 믿었던 것과 실제」 7항목과 1:1 이다(2026-08-18 대조).
        늘어난 둘은 §6(천장 5,600 이 환경 오염이었다는 것)과 §7(그걸 8/16 재측정으로 닫은 것)이고,
        K4 줄은 결론이 아니라 근거가 바뀌었다 — '재현 안 됨' 에서 '이득도 없음' 으로. */
     ab:[["천장 5,500 (착시)","","~5,000","분배 쏠림",-1],
         /* 한때 '7단계 연쇄' 로 세었다. 그 수는 결론 페이지가 갖고 있던 '이동 경로' 목록을 센
            것인데, 2026-08-18 개편에서 그 목록이 빠지면서 셀 근거가 사라졌다. 숫자를 뺀다 —
            없는 것을 세지 않는다. 경로 자체는 카드 번호 순서가 이미 말한다. */
         ["병목 위치 (뒤집힘)","","단계마다 옮겨감","그때마다 참",0],
    ...
    {s:1, loc:"2", n:"SendQ Lock-Free", st:"rj",
     r:"이득 없이 비용만 늘었다",
     ...
     src:{bm:"notion#끄고", am:"notion#켜고"},
     /* ①② 가 송신 경로가 아니라 틱 예산에 서는 이유(2026-08-04에 옮겼다).
        이 둘은 Send 워커가 생기기 전 실험이다 — 그때는 송신도 게임 스레드가 했고, ③ 이 떼어낸다
        (③ 카드가 "게임 루프의 송신 비중 74.5% → 0.08%" 라고 적은 그 74.5% 안에서 잰 값들이다).
        지표도 넷 중 셋이 게임 스레드 시간이다(broadcast_sync/틱 · game_logic/틱 · 넣기 1회당).
        지도는 '지금 구조' 를 그리므로, 송신 경로에 세우면 그 시점에 없던 부품 위에 서게 된다.
        ⚠ ①② 와 ③ 이 갈리는 것은 손실이 아니라 그 자체가 이야기다 — ①② 는 게임 스레드 **안에서**
          송신 비용을 줄이려 한 것이고 ③ 은 그 일을 **밖으로 뗀** 것이다. 순서는 카드 번호가 말한다.
        ❌ 옮기는 대신 화면에 사정을 적어 보는 안이 셋 반려됐다 — 상자 둘 칠하기(톤을 해친다) ·
          계기판 라벨 줄에 한 줄 · 상자 안 글자(9.5px 판독 하한이고 지면 테를 덮는다).
          지도 아래 캡션(.bn-cap)은 그 전에 이미 걷어낸 자리라 되살리지 않았다.
          셋 다 실패한 것이 '자리가 안 맞다' 는 신호였다 — 자리가 맞으면 설명이 필요 없다. */
     ab:[["broadcast_sync/틱","2.59","2.82 ms","+9%",-1],
         ["game_logic/틱","3.73","4.31 ms","+15%",-1],
         ["넣기 1회당","849","911 ns","+62 ns",-1],
         ["틱 p50","~30","~31 ms","±0%",0]]},

    {s:2, loc:"2", n:"Send Coalescing", st:"ok",
    ...
    {s:3, loc:"3", n:"SendThread 분리", st:"ok",
     ...
     src:{bm:"notion#끄고", am:"notion#켜고"},
     /* 평균 패킷 19.7 은 이 실험에서 잰 값이 아니다 — `avg_pkt_bytes` 지표가 2026-06-22 에 생겼는데
        이 런은 6/15~16 이라 원본에 그 칸이 없다. 같은 프로토콜을 쓴 ①② 의 `.prom` 실측(19.67~19.68)
        에서 빌려 온 값이고, 이 판 전체에서 **유일한 가정**이다(`_work/MEASURED_BASELINE.md`).
        그래서 계기판 왼쪽에 '당시 미계측' 을 세운다 — 값은 있으나 이 실험이 잰 것이 아니라는 표시다.
        되찾을 길은 없다 : 재측정하려면 옛 커밋으로 되돌려야 하는데 그 시절 측정 환경(별도 PC + WAN)
        자체가 없어졌고, 역산은 순환이다(41 MB/s 가 19.7 을 곱해 만든 값이라). */
     ldnote:{pkt:"당시 미계측"},
    ...
    {s:4, loc:"3", n:"SO_SNDBUF=0", st:"rj",
     ...
     src:{bm:"notion#끄고", am:"notion#켜고"},
     ldnote:{pkt:"당시 미계측"},   /* ③ 과 같은 사정 — 이 런도 `avg_pkt_bytes` 도입(6/22) 전이다 */
     ab:[["틱 평균","15.37","13.53 ms","−12%",1],
         ["시스템 전체 CPU","0.442","0.773 코어","+75%",-1],
         ["IOCP 워커 CPU","0.078","0.455 코어","5.8배",-1],
         ["송신 경합","0","12,422 /s","발생",-1]]},

    /* 이름은 진단 결과('회선 상한')를 앞에 둔다 — 붕괴는 현상이고 답은 회선이다(아래 r 과 같은 말).
       뒤의 1,500 은 시도값이라 남긴다. 계기판 동접이 833 이므로 둘이 한 화면에서 짝이 된다. */
    {s:5, loc:"4", n:"회선 상한 — 1,500", st:"dg",
     r:"서버가 아니라 회선이 천장이었다",
    ...
    {s:7, loc:"1", n:"IOCP 워커 12 → 4", st:"ok",
     ...
     src:{bm:"C4000_WT12_K2_r3", am:"C4000_WT4_K2_r1"},
     hint:"동접 2,500→4,000 스윕의 4,000 지점",
     abh:"워커12·송신2 → 워커4·송신2",
     ab:[["왕복 p99","143","102 ms","−28%",1],
         ["주기 평균","40.3","35.1 ms","−13%",1],
         /* 왼쪽이 48~185 였다. 노션이 적은 12×2 의 3회 왕복은 102 / 185 / 140 이라 48 은
            어디에도 없는 수다(2026-08-18 전수 대조). 실제 최소~최대로 맞춘다. */
         ["3회 편차","102~185","0.2 ms","재현성",1],
    ...
    /* '브로드캐스트' 를 뗀다 — 수신섹터 묶음이 원래 브로드캐스트를 줄이는 일이라 겹치는 말이고,
       이 실험은 이미 '수신섹터 묶음' 으로 부르기로 한 이름이다. */
    {s:9, loc:"2", n:"수신섹터 묶음", st:"ok",
    ...
    {s:10, loc:"3", n:"송신 워커 K2 → K3", st:"ok",
     ...
     /* 송신 메시지는 노션이 K 별로 안 나눴다 — 측정부하에 '약 1,000만/s' 하나뿐이다(K 가 바꾸는 것은
        분배뿐이라 통제 지표로 묶어 적은 것). 그래서 같은 표의 송신량과 평균 패킷으로 역산한다:
          K2 = 938 ÷ 93.4 B = 1,004만 · K3 = 940 ÷ 93.4 B = 1,006만
        한때 1,001만 → 1,005만 이었는데 출처가 없었고, 그 값으로는 계기판 아랫줄의 곱(송신 메시지 ×
        평균 패킷 = 송신량)이 938 이 아니라 935 로 어긋났다(2026-08-18 필드 대조에서 잡음). */
     bm:{ccu:5000,pps:10040000,send:938,tick:39.6}, am:{ccu:5000,pps:10060000,send:940,tick:39.7},
     src:{bm:"notion#끄고", am:"notion#켜고"},
     abh:"워커 2개 → 3개",
     ab:[["왕복 p99","187","138 ms","−26%",1],
         ["워커 최대 CPU","1.00","0.82 코어","포화 해소",1],
         ["워커당 밀린 양","2,500","1,667건","−33%",1],
         ["소켓 호출","100k","122k /s","+23%",-1]]},
    ...
    {s:11, loc:"2", n:"게임스레드 코어 격리", st:"rj",
     ...
     src:{bm:"CEIL_cc5200_A_off_r1", am:"CEIL_cc5200_B_on_r1"},
     hint:"천장 스윕 5,200 지점(부하 줄은 5,000 기준)",
     abh:"끄고 → 켜고",
     ab:[["접속 천장","~5,200","~5,200명","불변",-1],
         ["과부하(5,400) 틱 p99","57.6","97.8 ms","+70%",-1],
         /* 한때 노션이 이 칸을 Δ 한 값으로만 적어(3절 ① 표 `gameloop_cpu | −5%`) 이 자리를 비워
            두었는데, 지금은 노션이 0.531 → 0.503 (−5%) 로 절대값 쌍을 싣는다(2026-08-18 대조).
            실측 CSV(CEIL_cc5000 r1)는 0.5234 → 0.5076(−3.0%)로 다른데, 노션 쪽이 3rep 중앙값이라
            그렇다 — 카드는 노션 표기를 따른다. */
         ["게임루프 CPU","0.531","0.503 코어","−5%",1],
         ["5,400 송신 메시지/s","1,171만","1,154만","−1.5%",-1],
         ["5,400 평균 패킷","93.1","90.1 B","−3.2%",-1]]},

    /* ── 여기부터 넷은 digest 이후다(2026-07-29 ~ 08-13). 앞의 열하나가 '게임 스레드를 비우는'
       이야기였다면, 이 넷은 벽이 송신계로 넘어간 뒤의 이야기다 — 완료 수거(⑫)와 워커 수(⑭)를
       차례로 쳐 보고 둘 다 기각된 뒤, 남은 변수가 세션당 바이트라는 것을 ⑮ 가 보여 준다. ── */
    ...
    {s:12, loc:"1", n:"완료 통지 묶기", st:"rj",
     ...
     /* 한때 이 카드만 계기판이 통째로 비어 있었다 — 그 페이지가 부하를 완료율·워커 CPU 로만
        적어 트래픽 축이 없었기 때문이다. 2026-08-19 에 실측 CSV(`GQEX_4800_{GQCS,EX64}_r1/r2`
        2rep 평균)에서 여섯 칸을 되찾아 노션 측정부하에도 실었다.
        ※ 여섯 칸이 전부 ±0% 로 서는 것이 이 카드의 그림이다 — '완료를 꺼내는 방식만 바꿨고
          보내는 일의 양은 그대로' 라는 통제가 숫자로 보인다. 값이 안 움직이는 것이 결과다. */
     bm:{ccu:4800,pps:9707000,send:913,tick:39.2},
     am:{ccu:4800,pps:9711000,send:912,tick:39.3},
     src:{bm:"GQEX_4800_GQCS", am:"GQEX_4800_EX64"},
     hint:"세 점(2,000·4,000·4,800) 중 4,800 지점",
    ...
    /* 이름이 카드 폭을 넘겨 '멤버십 통보도 dig…' 로 잘렸다(2026-08-18 헤드리스 확인).
       ⑧ '멤버십 팬아웃 묶음' 과 짝이 되게 줄인다 — 무엇을 접었나(통보)는 계기판 첫 줄이 말한다. */
    {s:13, loc:"2", n:"멤버십 digest", st:"ok",
    ...
    {s:14, loc:"3", n:"송신 워커 K4·K5", st:"rj",
     ...
     /* 이 카드가 옛 'WT4+K4 금지 조합' 의 최종 판정이다 — 기각은 그대로인데 근거가 바뀌었다.
        한때 '배달 −16% 손해' 였던 것이 깨끗한 환경에서 재현되지 않아(평평 ±0.8%) '이득 없음' 으로
        교체됐다. 자리 0 의 정정 이력 카드도 같은 말을 한다. */
     /* 틱 p99 와 소켓 호출은 노션이 안 적었다(1절 표가 tick_avg 와 syscall당 바이트만 싣는다).
        실측 CSV 에서 채웠다 — `RMB_IOCP_D1_CC5600`(K3) → `RMC4_IOCP_D1_CC5600`(K4) 2rep 평균.
        같은 런의 송신량이 1,240.65 → 1,248.59 로 노션 1,240 → 1,249 와 맞아 라벨이 확인된다. */
     /* 송신 메시지는 양팔이 같은 값(1,305만)이었는데, 그러면 계기판이 자기모순이 된다 —
        1,305만 × 94.9B = 1,239 인데 바로 옆 송신량 칸은 1,249 를 세우기 때문이다.
        노션 표기(만 단위 1,305만)가 K3 값이라 K4 팔에도 그대로 붙어 있던 것이고, 같은 CSV 에
        K4 값이 따로 있다 : K3 1,305.5만 → K4 1,315.5만. 이 값이라야 곱셈이 닫힌다
        (1,305.5만 × 95.04 = 1,240.7 · 1,315.5만 × 94.91 = 1,248.6 — 옆 칸과 일치). */
     bm:{ccu:5600,pps:13055000,send:1240,tick:19.9},
     am:{ccu:5600,pps:13155000,send:1249,tick:19.9},
     src:{bm:"RMB_IOCP_D1_CC5600", am:"RMC4_IOCP_D1_CC5600"},
     hint:"안정 구간 대조 — 경계 6,700+ 에서의 K4 는 미측정",
     abh:"워커 3개 → 4개",
     /* Δ 는 노션 1절 표가 적어 둔 값 그대로다. 한때 넷을 다 '±0%' 로 적었는데, 그건 본문의
        '평평' 이라는 서술을 옮긴 것이지 표의 수가 아니었다(2026-08-18 전수 대조에서 잡음).
        틱만 노션이 Δ 를 안 줬다 — K3 11.7 → K4 12.2 → K5 11.4 로 오르내려 방향이 없어서다.
        그 요동 자체가 'K 는 변수가 아니다' 의 증거라 K5 값을 같이 적는다. */
    ...
    /* ⑮ 는 자리 4(회선)에 선다. 천장을 민 메커니즘이 송신큐 넘침 소멸이라 자리 3 도 후보였는데,
       노션이 이 실험을 '10G 회선 환산 142% → 112%' 로 앞세우고, ⑤ 이후 한 장뿐이던 회선 자리에
       두 번째 카드가 서면 '바깥이 천장이던 자리를 다시 밀었다' 가 지도에서 읽힌다(2026-08-18 결정). */
    /* '(9B)' 를 뗀다 — 이것도 카드 폭에서 잘렸고, 엔트리 크기는 아래 성과 줄과 계기판이 말한다. */
    {s:15, loc:"4", n:"좌표를 눈금으로", st:"ok",
     ...
     /* 2026-08-20 에 노션이 이 실험의 A/B 를 5,600 → **6,600** 으로 다시 쟀고, 여섯 칸을 그 지점으로
        옮겼다. 이 프로젝트의 관행은 '직전 실험이 확보한 천장까지 밀어 다시 벽에 닿는 자리에서 A/B'
        인데(Coalescing 이 '천장을 확보해 1,000 까지 올리자' 로 시작한 그 패턴), digest 가 천장을
        6,600 으로 민 뒤에도 이 카드만 5,600 에 남아 있었다 — 그 어긋남을 메운 재측정이다.
        옛 5,600 값(1,240 → 970 MB/s)은 ⑭ 가 같은 런으로 계속 들고 있으므로 화면에서 사라지지 않는다.
        새 부하점에서는 노션 3절 표가 **틱 p99 를 직접 싣는다**(38.5 → 29.9) — 5,600 시절에는 A/B 축이
        바이트뿐이라 틱을 안 적어 CSV 에서 대리로 채웠는데, 그 우회가 이제 필요 없다.
        곱셈도 닫힌다 : 1,744만 × 95.9B = 1,673 · 1,762만 × 75.2B = 1,325 ≈ 1,326. */
     bm:{ccu:6600,pps:17440000,send:1673,tick:38.5},
     am:{ccu:6600,pps:17620000,send:1326,tick:29.9},
     src:{bm:"notion:3bb16a0b9f598049a873fad65fc1aeff#3절·14B", am:"notion:3bb16a0b9f598049a873fad65fc1aeff#3절·9B"},
     abh:"14B → 9B",
    ...
    {s:21, loc:"1", n:"Worker→Game SwapQ", st:"na",
     r:"실측 데이터 없음",
     ...
     bm:null, am:null, src:{bm:null,am:null}},

    {s:22, loc:"1", n:"세션당 I/O 1회 + Timing Wheel", st:"na",
     r:"실측 데이터 없음",
     m:"세션마다 Recv/Send 를 1회로 제한 · 타임아웃은 바퀴를 돌려 O(1) 로",
     ex:"구현은 들어가 있으나 A/B 로 잰 적이 없다. 노션 [병목지점 개선] 에 해당 실측 페이지가 없고 Monitoring/metrics_out 에도 로그가 없다",
     bm:null, am:null, src:{bm:null,am:null}},

    {s:23, loc:"5", n:"dirty 선별 batch 저장", st:"na",
     r:"실측 데이터 없음",
     m:"바뀐 것만 골라 주기마다 묶어 UPSERT — 게임 스레드는 기다리지 않는다",
     ex:"구현은 들어가 있으나 A/B 로 잰 적이 없다. 노션 [병목지점 개선] 에 해당 실측 페이지가 없고 Monitoring/metrics_out 에도 로그가 없다",
     bm:null, am:null, src:{bm:null,am:null}},
  ];
  /* ═══ 카드 열여섯 장 = 노션 실측 보고서 열여섯 편 ═══
     이 순서가 곧 카드 번호(①~⑯)이고, 병목이 옮겨간 순서다 — 노션 DB "병목지점 개선" 의
     작성 순서와 같다(동접 200 에서 7,200 까지 부하를 올리며 잰 차례).
     id 는 대시 없는 32자다. 설계 문서는 이 id 를 NOTION 뒤에 그대로 붙여 그 페이지만 전체창으로
     열지만, 실측 보고서는 상위 페이지 뒤에 `?p=<id>&pm=c` 로 붙인다(아래 NOTION_HOME) —
     노션이 상위를 깔고 그 위에 작은 창으로 얹는 주소다. 규칙을 갈라 두는 이유는 그 작은 창이
     닫혔을 때 나머지 열다섯 편이 있는 목록에 떨어지기 때문이다. 설계 문서는 여덟 편이 이미
     한 화면(1-1 구역 카드)에 있어 그럴 이유가 없다.
     EXPS 항목에 직접 적지 않고 여기서 붙이는 이유: 이 표가 '카드에 서는 열여섯' 의 정본이라
  ...
  NOTION_EXP.forEach(function(r,i){
    ...
    if(e){ e.no=i+1; e.nt=NOTION+NOTION_HOME+"?p="+r[1]+"&pm=c"; }
  });
  ...
  /* ═══ 측정 부하 — 노션 각 페이지 콜아웃의 '측정부하' 토글에서 옮겨온 값 ═══
     ...
     팬아웃에 켜고 값이 없는 것은 그것이 통제 지표이기 때문이다 — 열여섯 실험 전부 OFF/ON 을
  ...
  const LOAD=[
    /* s:0 은 카드가 아니라 **눈금**이다(bneck.js SCALE). 현황판 am 과 같은 7,200 유효 2rep
       이라야 계기판 여섯 칸의 100% 가 한 지점에서 온다 — 이 행이 없으면 팬아웃·평균 패킷·
       소켓 호출만 결론 카드(5,600)의 자를 쓰게 되어 눈금 기준이 갈린다. */
    [0,   1326, 74.5, null, 158000, null],
    ...
    /* ⑩ 부하 줄과 3절 표가 K2 100k · K3 122k 로 같은 값을 적는다. 이 카드의 A/B 가 K2 → K3 이므로
       범위가 아니라 그 둘이 맞다. 한때 왼쪽이 99k 였는데, 그때 노션 표기가 99k 였고 그 뒤
       100k 로 손질됐다(2026-08-18 대조). */
    [10,   923, 93.4, null, 100000, 122000],
    ...
    /* ⑫ 는 노션이 트래픽 축을 안 적어 한동안 행이 없었다 — 2026-08-19 에 실측 CSV 에서 되찾아
       노션에도 실었다(`GQEX_4800_{GQCS,EX64}` 2rep 평균).
       ⑭ 는 K 별 소켓 호출·켜고 평균 패킷을 안 적었다(INI 한 줄만 바꾼 실험이라 부하가 공통). */
    [12,   890, 94.0, 93.9, 117616, 117608],
    [13,  1036, 93.5, 95.0, 134500, 135200],
    [14,  1037, 95.0,   94.9, 135070, 136455],
    /* ⑮ 와 ⑯ 은 2026-08-20 재측정으로 부하점이 6,600 이다 — 팬아웃·평균 패킷·소켓 호출도 전부
       그 지점 값이라 ⑭(5,600)와 나란히 두면 세 줄이 서로 다른 부하를 말한다. 그게 맞다.
       옛 5,600 줄(1,037 · 95.0 → 74.6 · 135,070 → 136,225)은 바로 위 ⑭ 가 그대로 들고 있다. */
    [15,  1218, 95.9, 75.2, 150500, 156900],
    [0.2, 1218, 75.2, null, 156900,   null]
  ];
  LOAD.forEach(function(r){
    const e=EXPS.filter(function(x){ return x.s===r[0]; })[0];
    if(e) e.ld={fan:r[1], pkt:r[2], pkt2:r[3], sock:r[4], sock2:r[5]};
  });
  /* ═══ 출처 표 — 위 값들이 '어느 런 · 어느 페이지' 에서 왔는가 ═══
     근거는 전부 위 주석들이 이미 적어 둔 것이다. 사람이 읽는 설명은 그대로 두고,
     기계가 물어볼 수 있게 라벨만 데이터로 꺼냈다 — 검사기(`_work/verify.mjs`)가 이걸 읽는다.
     같은 문자열을 가리키는 값끼리는 같아야 한다 — 그것이 이 표를 둔 이유다(⑭⑮ 가 같은 런을 다른 숫자로 말하던 일).

     라벨 문법
       "RMB_IOCP_D1_CC5600_r1"   실측 CSV 의 그 rep 하나
       "RMB_IOCP_D1_CC5600"      rep 접미사를 안 붙이면 그 런의 rep 전체 평균
       "A_r1+B_r1"               rep 을 골라 평균한 것(현황판의 유효 2rep 이 이 꼴)
       "notion#끄고"             이 카드의 노션 보고서(위 NOTION_EXP 가 id 를 갖고 있다) 의 끄고 열
       "notion:<id>#<절>"      다른 페이지가 근거일 때만 id 를 직접 적는다
       null                      그 팔이 없거나 그 값을 안 적은 것

     칸 뜻: [s, 끄고 출처, 켜고 출처]. 켜고가 null 인 행은 노션이 '변경 후' 를 안 적은 행이고,
     그때 계기판은 끄고 값 하나만 세우므로(paintDash 의 has2) 그 칸의 출처도 끄고 쪽이다. */
  const LOAD_SRC=[
    [0,    "RMDU_IOCP_D1_CC7200_r1+RMDU3_IOCP_D1_CC7200_r1", null],
    [1,    "notion#끄고", "notion#켜고"],
    [2,    "notion#끄고", "notion#켜고"],
    [3,    "notion#끄고", null],
    [4,    "notion#끄고", "notion#켜고"],
    [5,    "notion#끄고", null],
    [6,    "notion#끄고", "notion#켜고"],
    [7,    "C4000_WT12_K2_r3", "C4000_WT4_K2_r1"],
    [8,    "notion#끄고", "notion#켜고"],
    [9,    "notion#끄고", "notion#켜고"],
    [10,   "notion#끄고", "notion#켜고"],
    [11,   "CEIL_cc5200_A_off_r1", "CEIL_cc5200_B_on_r1"],
    [12,   "GQEX_4800_GQCS", "GQEX_4800_EX64"],
    [13,   "notion#끄고", "notion#켜고"],
    [14,   "RMB_IOCP_D1_CC5600", "RMC4_IOCP_D1_CC5600"],
    /* ⑮ 는 6,600 재측정(2026-08-20)으로 옮겼고, 부하 세 칸도 노션 3절 표에서 바로 온다 —
       5,600 시절처럼 CSV 를 대리 출처로 쓸 일이 없어졌다. 아래 ⑯ 과 '켜고' 출처가 같은데,
       두 카드가 같은 런을 말하므로 값도 같아야 한다(검사기 R3 가 그 짝을 본다). */
    [15,   "notion:3bb16a0b9f598049a873fad65fc1aeff#3절·14B", "notion:3bb16a0b9f598049a873fad65fc1aeff#3절·9B"],
    [0.2,  "notion:3bb16a0b9f598049a873fad65fc1aeff#3절·9B", null]
  ];
  LOAD_SRC.forEach(function(r){
    const e=EXPS.filter(function(x){ return x.s===r[0]; })[0];
    if(e) e.ldsrc={off:r[1], on:r[2]};
  });
  ...
  /* 카드 안을 채우는 일은 이제 없다 — 열여섯 장은 bneck.js 가 한 번 지어 두고 그대로 서 있는다.
  ...
  /* ═══ 핀 = 병목이 앉은 자리 ═══
     가르는 기준은 하나다 — **그 실험을 돌릴 때 아팠던 것이 무엇이냐**. 수신(IOCP 워커) · 게임 루프 ·
     송신(Send 워커) · 저장(DB 워커), 그리고 서버 밖의 회선. 이 다섯이 전부다.
     '무슨 일이냐' 가 아니라 '그때 아팠던 데' 인 것이 ①② 를 가른다 — 이름은 Send 로 시작하지만
     그 시절 송신은 게임 스레드의 일이었다(자세한 사연은 위 EXPS 의 ① 주석).

     한때 기준이 섞여 있었다(경로 셋 · 동작 둘 · 실험 이름 하나). 그래서 생긴 어긋남이 넷이다.
       · 자리 이름이 실험 하나에 매였다 — 자리 '멤버십 팬아웃' 에 ⑪ 코어 격리가 같이 서서,
         그 카드를 고르면 지도가 카드와 다른 말을 했다(팬아웃을 건드린 실험이 아니다).
       · ⑦ IOCP 워커 12→4 가 송신 경로에 붙어 있었다 — 수신 워커 수를 잰 실험이다.
       · ⑤ 회선 상한도 송신 경로였다 — 결론이 '서버가 아니라 회선' 인데 서버 부품 위에 섰다.
       · ①② 가 송신 경로였다 — 그 시점에는 아직 없던 부품(Send 워커) 위에 서 있었다.
     핀 이름은 노드 라벨과 겹치지 않게 짓는다(게임 루프 옆에 또 게임 루프 금지 → '틱 예산') */
  const HOT="#ff6b52";
  /* 좌표는 평면 기준이다 — 핀은 평면화가 거의 끝난 뒤에 꽂히므로 도중 자세를 따라다닐 필요가 없다.
     지면에 놓인 테는 평면에서 정원이 된다(아이소의 rx:ry=2:1 눌린 타원이 아니라).
     '틱 예산' 만 막대를 76 으로 세운 것은, 게임 루프 라벨이 그 자리 오른쪽에 있어 배지 이름표와 붙기 때문. */
  /* why = 이 자리가 왜 병목이었나. ⚠ 지금 화면에 나오는 곳이 없다 — 핀 클릭을 걷어내면서
     이 줄을 읽던 카드가 사라졌다(2026-08-04). 자리 뜻의 기록으로 남겨 둔다. */
  const PINS=[
    ...
    {n:"0", name:"현재", nopin:true, u:0, v:0, h:0, rx:0, side:1,
     why:"카드마다 찍힌 동접은 그 실험을 돌린 시점의 부하다. 마지막 카드의 5,600 을 지금 값으로 읽지 않도록, 현재 참인 값을 여기 따로 둔다."},
    /* 순서는 패킷이 지나가는 순서다 — 받고(1) · 굴리고(2) · 보내고(3) · 회선(4), 그리고 저장(5).
       핀이 떨어지는 순서이기도 하다(아래 animationDelay 가 이 배열 인덱스를 쓴다).

       node = 이 자리가 앉아 있는 '부품'. 핀은 점이라 아무리 키워도 점이지만, 부품 발자국은
       면적을 가지므로 '무엇이 병목이었나' 가 지도에서 바로 읽힌다(아래 .pin-slab).
       ※ dbw 는 핀이 (273,148) 이었다 — 발자국(277~343, 159~181) **바깥 모서리**라, 지면 테가
         판을 벗어나 DB 워커 부제('통째 인출 · 배치 UPSERT') 글자를 가로질렀다. 판 중앙(310,170)
         으로 옮겼다(2026-08-04). DB 워커는 몸통이 히트박스 밖(판 뒤 +v 27)이라 원래 좌표만 보면
         어긋나는 노드다 — 이 자리를 다시 만질 일이 생기면 판 좌표부터 확인할 것. */
    {n:"1", name:"수신 길목",  u:190, v:50,  h:56, rx:15, side:-1, node:"iocp",
     why:"완료 통지를 받아 파싱하는 길목 — 워커를 몇으로 둘지가 여기서 정해진다. 많으면 나눠 갖는 게 아니라 서로 밀린다."},
    /* 일곱 장이 여기 선다 — ①②(송신을 게임 스레드가 하던 시절) · ⑥⑨(브로드캐스트 복사) ·
       ⑧⑬(팬아웃·통보) · ⑪(코어 격리). 병목이 가장 오래 머문 자리라 카드도 제일 두껍다.
       ①② 를 여기로 옮긴 사연은 위 EXPS 의 ① 주석에 있다.
       ※ '병목은 게임 루프 단일 코어' 는 ⑬ 이전의 결론이었다 — ⑬ 이 그 벽을 걷어내며 병목이
         송신계로 넘어갔고, 그래서 자리 3·4 에도 카드가 붙었다(⑭⑮). 이 자리의 두께는 이제
         '지금 병목' 이 아니라 '가장 오래 병목이던 곳' 을 말한다. */
    {n:"2", name:"틱 예산",    u:410, v:50,  h:76, rx:20, side:1,  node:"game",
     why:"한 스레드가 25fps 안에 다 끝내야 하는 자리 — 대상 계산이든 복사든, 송신조차 한때 여기서 예산을 먹었다."},
    /* ❌ 이 자리만 상자 둘(송신 큐 + Send 워커)을 칠해 본 적이 있다 — 그때는 ①② 도 이 자리라
       '큐 실험과 워커 실험이 한 자리에 선다' 를 그림으로 맞추려던 것이었다. 반려(2026-08-04):
       강조는 되지만 톤을 해친다. 두 상자가 화면에서 멀어(사이에 GAME 바닥글자) 경로 하나가 아니라
       '두 군데' 로 읽히는 것도 같이 걸렸다. 되살릴 일이 생기면 node 를 배열로 받게 하면 된다. */
    {n:"3", name:"송신 경로",  u:190, v:-70, h:60, rx:15, side:-1, node:"send",
     why:"송신을 게임 틱에서 떼어낸 뒤의 자리 — 전용 스레드가 얼마나·몇으로 보낼지가 여기서 정해진다(③ 이후)."},
    /* 회선만 node 가 없다. 서버 부품이 아니라 **구간**(send → clients 복귀 레일)이라 깔 발자국이
       없다 — 클라 상자에 얹으면 '클라이언트가 병목' 으로 읽히고, 송신 워커에 얹으면 이 카드가
       뒤집은 결론('서버가 아니라 회선')을 지도가 되돌려 놓는다. 면적이 없는 것이 정확하다.
       좌표는 그 레일의 수평 구간 위다 — send 중심(190,-70) 에서 clients 중심(-14,50) 으로 갈 때
       v=-70 을 따라 왼쪽으로 흐르는 구간(scene.js 위 RAILS 의 pts u:-14,v:-70). */
    {n:"4", name:"회선 상한",  u:75,  v:-70, h:52, rx:13, side:-1,
     why:"서버가 아니라 바깥이 천장이던 자리 — 틱은 예산 안인데 왕복만 1초로 벌어졌다(WAN 상한 474 Mbps)."},
    {n:"5", name:"저장 경로",  u:310, v:170, h:64, rx:13, side:1,  node:"dbw",
  ...
  /* 자리별 부제(채택 n · 기각 n)는 손으로 적지 않고 실험 배열에서 센다 —
     ...
     ※ na 를 '미측정' 이라 적었었다. 계측은 서버 프로세스 전체를 대상으로 돌았고(카드들이
       game_logic/틱 · network/틱 · IOCP 워커 CPU 를 나란히 놓고 비교한다) 이 셋은 거기서
       병목으로 안 올라온 자리다 — 그래서 전용 A/B 를 돌릴 이유가 없었다. '미측정' 은
       '아직 안 해 본 곳' 으로 읽혀 사실과 어긋난다. 없는 것은 이 항목 전용 실험이지 계측이 아니다. */
  PINS.forEach(function(p){
    const c={ok:0,rj:0,dg:0,na:0,now:0,sum:0,fix:0};
    EXPS.forEach(function(e){ if(e.loc===p.n) c[e.st]++; });
    p.sub=[c.ok&&"채택 "+c.ok, c.rj&&"기각 "+c.rj, c.dg&&"진단 "+c.dg, c.na&&"병목 아님 "+c.na,
  ...
  PINS.forEach(function(p,i){
    ...
    /* 핀은 누르는 것이 아니다 — 자리를 짚는 표시일 뿐이다(2026-08-04).
       한때 '이 자리에서 잰 첫 실험' 으로 가는 입구였는데, 그 실험 카드가 오른쪽에 열여섯 장 다 펼쳐져
    ...
    /* 고른 자리가 앉은 부품의 발자국. 핀보다 먼저(맨 아래) 넣어 지도 위에 깔리게 한다.
       평면 좌표로 한 번 계산해 두면 되는 이유는 핀과 같다 — 이 판이 서는 시점에는 땅이 이미 다 펴져 있다. */
    if(p.node && byName[p.node] && LAYOUT[p.node]){
      const nd=byName[p.node], NL=LAYOUT[p.node], nh=nd.hit;
      const su=NL.u+nh[0], sv=NL.v+nh[1];
      g.appendChild(el("polygon",{class:"pin-slab", points:
        [[su,sv],[su+nh[2],sv],[su+nh[2],sv+nh[3]],[su,sv+nh[3]]]
          .map(function(c){ return fx(c[0])+","+fy(c[1]); }).join(" ")}));
    }
    /* 땅이 다 펴진 뒤에 꽂는다 — 아직 기울어 있는 땅에 평면 좌표의 핀이 먼저 서면 자리가 어긋나 보인다.
       꽂는 순서는 배지 번호 순서다. 카드 1·2·3 과 짝이라 세는 순서가 먼저다. */
    /* 1.34 → 1.62s. 핀 드랍이 평면화(0.86~1.58s, 매 프레임 도형 950개)와 겹쳐 있었다 —
       그 구간에서 굽는 쪽이 못 따라와 '뚝뚝' 끊겨 보였다(계측으로는 안 잡힌다: 유실 1프레임 ·
       long-animation-frame 0 · 메인 스레드는 놀고 있었다. 2026-08-04 실측).
       카메라를 평면화에서 떼어낸 것과 같은 수법이다 — 겹치지 않게 뒤로 민다. */
    g.style.animationDelay=(1.62+i*0.08)+"s";
    ...
    /* 테두리는 배경색이다. 한때 흰끼 도는 살구(#ffe0d6)였는데, 이 지도에 흰색은 여기뿐이라
       배지가 지도 위에 붙인 스티커처럼 떴다. 어두운 테를 두르면 배지와 바깥 테(halo) 사이가
       벌어져 겹링 구조가 더 또렷해지고, 팔레트(붉은 계열 + 남색)를 벗어나는 색이 없어진다. */
    g.appendChild(el("circle",{class:"pin-badge",cx:x,cy:yt,r:11,fill:HOT,stroke:"#0f1626","stroke-width":2.2}));
    ...
    /* 이름표를 배지에서 24 띄운다. 19 였을 때는 **고른 자리에서만** 끝 글자가 배지에 물렸다 —
       평소 halo(r 15)까지는 4 가 남지만, .lit 이 halo 를 1.18 배(17.7)로 키우면 여유가 1.3 뿐이라
       글자 획과 테가 맞닿는다('송신 경로' 의 로 · '회선 상한' 의 한). 켜진 자리가 그 자리에서
       가장 크고 밝은데 거기서 이름이 물리므로, 기준은 켜진 쪽으로 잡는다. */
    const tx=x+p.side*24, anch=p.side>0?"start":"end";
  ...
  /* 시계 (전부 클릭 기준):
       ...
       1.34s      카드 켜기    ← 합성 단계의 알파 한 값뿐
       1.62s      핀 드랍      ← **평면화가 끝난 뒤**(1.58s). 핀 좌표가 '평면 기준' 이라 순서를 지켜야 하고,
                                겹쳐 두면 그 구간에서 굽는 쪽이 못 따라와 끊겨 보인다(2026-08-04)
     ※ 끊김을 잡으려고 이 길이를 1200ms 로 늘려 본 적이 있는데, 원인은 길이가 아니라 카메라와 겹친 것이었다.
       카메라를 앞으로 뺀 뒤로는 720ms 로도 매끄럽다 — 원래 속도가 더 낫다고 판단해 되돌렸다.
     FLAT_DUR 을 건드리면 핀 지연(1.62)·카드 켜기(1340)·BN_TOTAL 을 같은 폭으로 함께 옮길 것.
     ※ BN_TOTAL 1860 → 2140 → 2260. 핀은 1.62s 부터 0.08s 간격으로 차례로 떨어지고 한 개가 0.30s 걸린다.
       지연은 배열 인덱스로 매기므로 자리 5 개인 지금 마지막 핀은 1.74s 에 시작해 2.04s 에 끝난다
       (자리가 여섯이던 시절엔 2.12s 였다 — 그때 이 값을 2140 으로 올렸다). 1860 에 .bn-enter 를
       떼면 그 핀은 공중에서 규칙을 잃고 제자리로 점프했다(꽂히다 만다).
       진입의 끝은 마지막 핀이 앉는 시각이다 — 자리를 더 늘리면 여기부터 볼 것. */
  const FLAT_AT=860, FLAT_DUR=720, BN_TOTAL=2260;   /* 마지막 핀 1.62+4×0.08+0.30 = 2.24s ≤ 2.26 */
  ...
  function prewarm(){
    ...
    pb.hidden=false;
    /* 오른쪽 실험 판은 패널을 열어 주는 것만으로는 안 데워진다 — 자기도 hidden 이라(.bncard[hidden]
       {display:none}) 예열 프레임에서 배치도 래스터도 안 된다. 그래서 첫 전환은 0.30s(짓기)와
       1.34s(켜기) 두 번에 걸쳐 이 판을 처음 굽고, 그 값이 그대로 프레임 드랍이 된다.
       실측(창 1568×810 · dpr 1 · 새 프로필, 두 번 반복): 첫 전환 +1.45s 에 150ms · 133ms 한 방
       (=8~9장 유실). blockingDuration 0 이라 JS 가 아니라 굽는 쪽이다. 여기서 두 프레임만 같이
       열어 두면 그 한 방이 사라진다(첫 전환 유실 프레임 10 → 1).
       opacity 까지 켜는 이유: 투명한 서브트리는 배치만 하고 그리기를 건너뛴다 — 자리만 잡아서는
       굽는 값이 그대로 남는다. 판 전체가 opacity .01 이라 이 두 프레임도 눈에는 안 띈다. */
    var bc=bnCard, bcOn=bc&&bc.classList.contains("on");
    if(bc){ bc.hidden=false; bc.classList.add("on"); }
    requestAnimationFrame(function(){ requestAnimationFrame(function(){
      ...
      /* 카드는 조건 없이 되돌린다 — 그 사이 탭이 눌렸다면 진입 애니가 이미 closeCard() 로
         같은 상태(숨김·on 없음)를 만들어 둔 뒤라, 여기서 다시 앉혀도 결과가 같다.
         켜 두고 나가면 진입 애니가 시작부터 카드를 보인 채로 돈다. */
      if(bc){ if(!bcOn) bc.classList.remove("on"); bc.hidden=true; }
    }); });
  }
