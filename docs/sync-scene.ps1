...
# 시작점을 216,172 → 243,191 로 옮겼다(= 그림이 왼쪽·위로 27·19 만큼 이동).
# 오른쪽 GAME 평면이 카드 밖으로 잘려 나가고 아래쪽 '클라이언트' 글자가 범례에 붙던 것을 함께 푼다.
# 이후 x 를 243 → 264 로 한 번 더 옮겼다. 헤드리스 실측(창 1440)으로 카드 안 좌우 여백이
# 왼쪽 84px · 오른쪽 30px 이라 그림이 오른쪽으로 쏠려 보였다 — 21칸(≈27px) 왼쪽으로 당겨 53:56 으로 맞춘다.
# 그다음 씬에 요각(YAW=−12)이 들어가면서 그림이 통째로 오른쪽·아래로 밀렸다(내용 좌상단 308,211 → 360,238).
# 크기는 그대로라 창을 같은 만큼 따라 옮기면 된다: 264,191 → 316,209.
# x 는 거기서 4 더 밀었다 — 요각으로 내용 폭이 767→776 칸으로 9 늘어 그만큼 오른쪽 여백이 깎였다(57:46).
$homeVB = 'viewBox="320 209 856 500"'
